...
abstract class AbstractALUTMR(implicit p: Parameters) extends CoreModule()(p) {
  val io = IO(new Bundle {
    ...
    val fn = Input(UInt(SZ_ALUTMR_FN.W))
...
class ALUTMR(implicit p: Parameters) extends AbstractALUTMR()(p) {
  ...
  val adder_out1 = io.in1 + in2_inv + isSub(io.fn)

  // SLT, SLTU
  val slt =
    Mux(io.in1(xLen-1) === io.in2(xLen-1), adder_out1(xLen-1),
    Mux(cmpUnsigned(io.fn), io.in2(xLen-1), io.in1(xLen-1)))
  cmp_out1 cmpInverted(io.fn) ^ Mux(cmpEq(io.fn), in1_xor_in2 === 0.U, slt)
  ...
  // MAX, MIN, MAXU, MINU
  val maxmin_out = Mux(cmp_out1, io.in2, io.in1)
  ...
  val out1 = MuxLookup(io.fn, shift_logic_cond)(Seq(
    FN_ADD -> adder_out1,
    FN_SUB -> adder_out1
  ...
  val in1_xor_in2 = io.in1 ^ in2_inv
  val in1_and_in2 = io.in1 & in2_inv
  val adder_out2 = io.in1 + in2_inv + isSub(io.fn)

  // SLT, SLTU
  val slt =
    Mux(io.in1(xLen-1) === io.in2(xLen-1), adder_out2(xLen-1),
    Mux(cmpUnsigned(io.fn), io.in2(xLen-1), io.in1(xLen-1)))
  cmp_out2 cmpInverted(io.fn) ^ Mux(cmpEq(io.fn), in1_xor_in2 === 0.U, slt)
  ...
  // MAX, MIN, MAXU, MINU
  val maxmin_out = Mux(cmp_out2, io.in2, io.in1)
  ...
  val out2 = MuxLookup(io.fn, shift_logic_cond)(Seq(
    FN_ADD -> adder_out2,
    FN_SUB -> adder_out2
  ) ++ (if (coreParams.useZbb) Seq(
    FN_UNARY -> unary,
  ...
  val in1_xor_in2 = io.in1 ^ in2_inv
  val in1_and_in2 = io.in1 & in2_inv
  val adder_out3 = io.in1 + in2_inv + isSub(io.fn)

  // SLT, SLTU
  val slt =
    Mux(io.in1(xLen-1) === io.in2(xLen-1), adder_out3(xLen-1),
    Mux(cmpUnsigned(io.fn), io.in2(xLen-1), io.in1(xLen-1)))
  cmp_out3 cmpInverted(io.fn) ^ Mux(cmpEq(io.fn), in1_xor_in2 === 0.U, slt)
  ...
  // MAX, MIN, MAXU, MINU
  val maxmin_out = Mux(cmp_out3, io.in2, io.in1)
  ...
  val out3 = MuxLookup(io.fn, shift_logic_cond)(Seq(
    FN_ADD -> adder_out3,
    FN_SUB -> adder_out3
  ) ++ (if (coreParams.useZbb) Seq(
    FN_UNARY -> unary,
